...
import { formatTimeout, getRulePatternFeedback, MenuSetting, normalizeRule, debounce, useNotifications, cn } from 'extension-shared';
import { deleteSiteRule, findConflictingSiteRule, hasValidPattern, settingsEqual, upsertSiteRule } from '../lib/settings';
import { DurationSelect } from '../components/durationSelect';
import { DEFAULT_SITE_RULE } from '../lib/util';
...
export function SiteFormView({
	...
	const persist = (
		...
		const isValid = hasValidPattern(next);
	...
	return (
		<div className="grid gap-2">
					...
					<MenuSetting>
						...
						<DurationSelect
							...
							onChange={(value) => persist(
								{ ...draft, timeoutMinutes: value },
								{ label: 'Unload', value: formatTimeout(value) },
								0,
							)}
						/>
					</MenuSetting>
			...
			<span className="text-text-500 text-sm">
				Override the global timeout per site. More specific patterns win, so{' '}
